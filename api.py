...
temperatures = [150.0 for _ in range(N_SENSORS)]
...
def update_temperature(old_temp):
    ...
    new_temp = max(149.75, min(150.25, new_temp))
...
def get_temperature():
    ...
    if max(temperatures) - min(temperatures) >= 1:
        temperatures = [t - 0.05 for t in temperatures] if max(temperatures) > 150 else [t + 0.05 for t in temperatures]
    
    payload = {"time": formatted_datetime, "num_sensors": f"{N_SENSORS}", "temperatures": [str(t) for t in temperatures]}
